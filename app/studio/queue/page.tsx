...
const QUEUE: QueueItem[] = [
  ...
  },
  {
    ...
    added: "2026-05-25",
    blurb:
      "Internal Chappie Studio infrastructure. The seven personas live as Multica squad agents on a project board; Sire sits in as the human override. Not a customer product, not resold — license SaaS-embed restriction doesn't apply (internal-org-use exemption). Phase 0 recon shipped 2026-05-26 (33k stars, v0.3.8, OpenClaw auto-detected as a first-class runtime). Phase 1 artifacts complete 2026-06-02: verified Docker image paths from Multica's GitHub SELF_HOSTING.md, custom Dockerfile.daemon that installs OpenClaw via npm + Multica CLI via official install script, ChappieTeam squad JSON (7 personas + Sire), Chappie voice skill, 3 real test issues. Deploys to Render: backend (Go) + frontend (Next.js) + custom daemon + managed Postgres = $28/mo. Sire's gate is the 6-step SIRE_DEPLOY.md.",
    ref: { label: "multica.ai", href: "https://multica.ai/" },
    workItems: [
      "✓ Phase 0 recon — Multica verified real (33k stars, v0.3.8 2026-05-25, OpenClaw auto-detected from PATH as a first-class runtime)",
      "✓ Self-host stack mapped — Node 20+ / pnpm 10.28+ / Go 1.26+ / Docker / Postgres 17 + pgvector",
      "✓ Squad routing confirmed LLM-judged (leader delegates via @-mention with role-description-aware roster); humans can be squad members",
      "✓ Autopilot syntax: standard 5-field cron + IANA timezone + webhook triggers + manual run",
      "✓ GitHub auth: read-only App on PRs + metadata — light trust ask, no write-scope PAT",
      "✓ License: internal-org-use exemption applies. We're not reselling, embedding, or third-party-distributing Multica.",
      "✓ Docker image paths verified from github.com/multica-ai/multica SELF_HOSTING.md: ghcr.io/multica-ai/multica-backend (Go) + ghcr.io/multica-ai/multica-frontend (Next.js). No prebuilt daemon image — we build it.",
      "✓ Daemon container resolved: custom Dockerfile.daemon FROM node:24-slim, installs OpenClaw via npm + Multica CLI via official Linux install script, runs `multica daemon start --foreground`",
      "✓ Phase 1 artifacts at ~/.openclaw/workspace/multica-phase-1/: docker-compose.yml, Dockerfile.daemon, chappie-team-squad.json, .agent_context/skills/chappie-voice.md, test-issues.md, SIRE_DEPLOY.md",
      "Sire's deploy: SIRE_DEPLOY.md — GitHub App → 4 Render services ($28/mo) → 8 env vars → import squad JSON → copy skills → open the 3 test issues, watch routing",
    ],
  },
  {
